...
class PriceModel extends Model
{
    ...
    //viene chiamato dal command UpdatePrices (task scheduler ogni 15 minuti
    ...
    */
    ...
    {
        $builder = $this->where('ticker', $ticker)
            ->where('mic', $mic);

        //applica il filtro di data in base al range selezionato dall'utente
        switch ($range) {
            case '3M':
                $builder->where('date >=', date('Y-m-d H:i:s', strtotime('-3 months')));//strtotime restiutisce data di 3 mesi fa
                break;
            case '6M':
        ...
        return $builder->orderBy('d', 'ASC')->findAll();
    }

    /*costruisce un array chiave=>valore con l'ultimo prezzo per ogni listing.
