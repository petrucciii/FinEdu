                ...
                <div class="d-flex gap-2 align-items-center">
                    <div class="input-group input-group-sm" style="width: 280px;">
                        <input type="text" id="searchInput" class="form-control"
                            placeholder="Cerca ticker, ISIN o società...">
                    ...
                    <select id="filterExchange" class="form-select form-select-sm" style="width: 250px;">
                        ...
                        <?php foreach ($exchanges as $ex): ?>
                            <option value="<?= esc($ex['mic']) ?>"><?= esc($ex['full_name']) ?> (<?= esc($ex['mic']) ?>)
                            </option>
                        <?php endforeach; ?>
                    </select>
                </div>
            ...
            <div class="modal-content">
                ...
                <form action="/PortfolioController/buy" method="post">
                    <div class="modal-body">
                        ...
                        <!--prezzo memorizzato dal js per il calcolo costo-->
                        ...
                        <div class="mb-3">
                            ...
                            <select name="portfolio_id" class="form-select" required>
                                <?php foreach ($userPortfolios as $pf): ?>
                                    <!-- manca validazione: valore ordine non deve superare quantita acquisdtata -->
                                    <option value="<?= (int) $pf['portfolio_id'] ?>">
                                        <?= esc($pf['name']) ?> — Liq. € <?= number_format((int) $pf['liquidity'], 0, ',', '.') ?>
                                    </option>
                                <?php endforeach; ?>
                            </select>
                        </div>
                        <div class="mb-3">
                            ...
                            <input type="number" name="quantity" id="buyQuantity" class="form-control" min="1" value="1"
                                required>
                        </div>
